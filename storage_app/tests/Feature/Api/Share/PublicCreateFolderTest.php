...
class PublicCreateFolderTest extends TestCase
{
    use RefreshDatabase, WithFaker, UserSettingsTestData, FolderTestData;
    /**
     * PublicCreateFolderTest.
     *
     * @return void
     */
    public function test_publicCreateFolderTest()
    {
        ...
        $dataTwo = $this->folderTestData($folderData->id);
        
        $this->withUnencryptedCookies($cookie)
                ->post('api/v1/sharing/create-folder/' . $sharedItem['data']['items'][0]['data']['attributes']['token'], $dataTwo)
                ->assertStatus(201);
    }
}
